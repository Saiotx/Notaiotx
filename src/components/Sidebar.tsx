...
export default function Sidebar({ isOpen, setIsOpen }: SidebarProps) {
  ...
  return (
    <div 
      className={`fixed md:relative top-0 left-0 w-64 h-screen bg-[#F8F9FA] flex flex-col border-r border-gray-200 shrink-0 z-40 transition-transform duration-300 ease-in-out ${
        isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
      }`}
    >
      {/* Mobile Close Button */}
      <div className="md:hidden flex justify-end p-2 border-b border-gray-200 bg-white">
         <button onClick={() => setIsOpen(false)} className="p-2 text-gray-500 hover:bg-gray-100 rounded-lg">
           <X className="w-5 h-5" />
         </button>
      </div>
      ...
      <div className="p-4 flex flex-col gap-4">
        ...
        <div className="flex items-center gap-2">
          <Link href="/notas" onClick={() => setIsOpen(false)} className="flex-1 bg-[#00a82d] text-white font-medium py-2 px-3 rounded-full flex items-center justify-center gap-2 hover:bg-[#008f26] shadow-sm">
      ...
      <div className="p-4 border-t border-gray-200 flex flex-col gap-4">
        <div 
          ...
          className="flex items-center gap-2 cursor-pointer hover:bg-gray-100 p-2 -mx-2 rounded-lg"
          title="Cerrar sesión"
        >
          <div className="w-6 h-6 rounded-full bg-teal-500 text-white flex items-center justify-center text-xs font-semibold overflow-hidden">
             {user?.photoURL ? <img src={user.photoURL} alt="User" className="w-full h-full object-cover" /> : user?.email?.charAt(0).toUpperCase() || 'S'}
          </div>
          <span className="text-xs text-gray-700 truncate font-medium">{user?.email || 'saioagartzia@gmail.com'}</span>
          <div className="ml-auto flex items-center gap-2">
            <span className="text-[10px] bg-red-100 text-red-800 px-1 py-0.5 rounded font-mono">Salir</span>
          </div>
        </div>
      </div>
    </div>
